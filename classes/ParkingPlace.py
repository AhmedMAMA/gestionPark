...
class ParkingPlace:
    ...
    def ParkingField(self):
        ...
        if line_img is not None:
            ...
            cv2.imwrite("placedetect.png",self.image)
        return line_img
    ...
    def detectePlace(self,lines_detected):
        ...
        for (x1,y1,x2,y2) in lines_detected:
            if abs(x1 -  x2) > 170 : # Seuil pour considérer une nouvelle ligne
                unique_lines.append((x1,y1,x2,y2))

        # Nombre de places = espaces entre les lignes
        print(f"Nombre de places détectées : {len(unique_lines)}")
...
def main(img):
    ...
    if result is not None:  # Vérifie si des lignes ont été détectées
        ...
        rem.detectePlace(lines)  # Détecte les places de parking
    else:
